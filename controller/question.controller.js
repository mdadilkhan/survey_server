    ...
    await usersCollection.updateOne(
      {
        ...
        "questionResponses.questionId": { $ne: questionId },
      },
      {
...
const getSurveyResultsByQuestionId = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message:
        "Question data and today's survey results retrieved successfully",
      questionId,
    ...
    console.error(
      "Error fetching question data and responses by question ID:",
      error
    );
    res.status(500).json({ message: "Server error" });
  }
...
const getLimitedUnderstandingJobOpportunities = async (req, res) => {
  ...
  try {
    ...
    const question1Response = user.questionResponses.find(
      (response) => response.questionId === "1"
    );
    const question2Response = user.questionResponses.find(
      (response) => response.questionId === "2"
    );
    const question3Response = user.questionResponses.find(
      (response) => response.questionId === "3"
    );
    ...
    if (question2Response) {
      const question2 = await db
        .collection("questions")
        .findOne({ questionId: "2" });
      for (let optionId of question2Response.optionSelected) {
        const option2 = question2.options.find(
    ...
    if (question3Response) {
      const question3 = await db
        .collection("questions")
        .findOne({ questionId: "3" });
      for (let optionId of question3Response.optionSelected) {
        const option3 = question3.options.find(
          (opt) => opt.optionId === optionId
        );
    ...
    const totalOutcomePoints =
      question1Outcome + question2Outcome + question3Outcome;
...
const getLackOfSkillsAndPreparedness = async (req, res) => {
  ...
  try {
    ...
    const question4Response = user.questionResponses.find(
      (response) => response.questionId === "4"
    );
    const question5Response = user.questionResponses.find(
      (response) => response.questionId === "5"
    );
    const question10Response = user.questionResponses.find(
      (response) => response.questionId === "10"
    );
    ...
    if (question10Response) {
      ...
      const question10 = await db
        .collection("questions")
        .findOne({ questionId: "10" });
      const option10 = question10.options.find(
        (opt) => opt.optionId === option
      );
    ...
    const totalOutcomePoints =
      question4Outcome + question5Outcome + question10Outcome;

    // Return the result
    res.status(200).json({
      message:
        "Outcome points for Lack of Skills and Preparedness calculated successfully",
      data: {
        question4Outcome,
        question5Outcome,
        question10Outcome,
...
const getConfusionAboutBranchesAndAlignment = async (req, res) => {
  ...
  try {
    ...
    const question6Response = user.questionResponses.find(
      (response) => response.questionId === "6"
    );
    const question13Response = user.questionResponses.find(
      (response) => response.questionId === "13"
    );
    ...
    if (question6Response) {
      ...
      const question6 = await db
        .collection("questions")
        .findOne({ questionId: "6" });
    ...
    if (question13Response) {
      ...
      const question13 = await db
        .collection("questions")
        .findOne({ questionId: "13" });
      const option13 = question13.options.find(
        (opt) => opt.optionId === option
      );
    ...
    const totalOutcomePoints =
      question2Outcome + question6Outcome + question13Outcome;

    // Return the result
    res.status(200).json({
      message:
        "Outcome points for Confusion About Branches & Alignment calculated successfully",
      data: {
        question2Outcome,
        question6Outcome,
        question13Outcome,
...
const getInternshipSelectionForJobReadiness = async (req, res) => {
  ...
  try {
    ...
    const question9Response = user.questionResponses.find(
      (response) => response.questionId === "9"
    );
    const question10Response = user.questionResponses.find(
      (response) => response.questionId === "10"
    );
    const question12Response = user.questionResponses.find(
      (response) => response.questionId === "12"
    );
    ...
    if (question9Response) {
      ...
      const question9 = await db
        .collection("questions")
        .findOne({ questionId: "9" });
    ...
    if (question12Response) {
      ...
      const question12 = await db
        .collection("questions")
        .findOne({ questionId: "12" });
      const option12 = question12.options.find(
        (opt) => opt.optionId === option
      );
    ...
    res.status(200).json({
      message:
        "Outcome points for Internship Selection for Job Readiness calculated successfully",
      data: {
        question5Outcome,
        question9Outcome,
        question10Outcome,
...
const getMismatchSalaryExpectations = async (req, res) => {
  ...
  try {
    ...
    const question7Response = user.questionResponses.find(
      (response) => response.questionId === "7"
    );
    const question8Response = user.questionResponses.find(
      (response) => response.questionId === "8"
    );
    ...
    if (question7Response) {
      ...
      const question7 = await db
        .collection("questions")
        .findOne({ questionId: "7" });
    ...
    if (question8Response) {
      ...
      const question8 = await db
        .collection("questions")
        .findOne({ questionId: "8" });
    ...
    res.status(200).json({
      message:
        "Outcome points for Mismatch Salary Expectations calculated successfully",
      data: {
        question7Outcome,
        question8Outcome,
        totalOutcomePoints,
...
const getInterestCareerSupportServices = async (req, res) => {
  ...
  try {
    ...
    const question11Response = user.questionResponses.find(
      (response) => response.questionId === "11"
    );
    const question13Response = user.questionResponses.find(
      (response) => response.questionId === "13"
    );
    ...
    if (question11Response) {
      ...
      const question11 = await db
        .collection("questions")
        .findOne({ questionId: "11" });
      const option11 = question11.options.find(
        (opt) => opt.optionId === option
      );
    ...
    const totalOutcomePoints =
      question11Outcome + question13Outcome + question10Outcome;

    // Return the result
    res.status(200).json({
      message:
        "Outcome points for Interest Career Support Services calculated successfully",
      data: {
        question11Outcome,
        question13Outcome,
        question10Outcome,
...
const getUniversityLimitedUnderstandingJobOpportunities = async (req, res) => {
  ...
  try {
    ...
    if (!users || users.length === 0) {
      return res
        .status(404)
        .json({ message: "No users found for the specified university" });
    }
    ...
    for (const user of users) {
      ...
      const question1Response = questionResponses.find(
        (response) => response.questionId === "1"
      );
      const question2Response = questionResponses.find(
        (response) => response.questionId === "2"
      );
      const question3Response = questionResponses.find(
        (response) => response.questionId === "3"
      );
      ...
      if (question1Response) {
        ...
        const question1 = await db
          .collection("questions")
          .findOne({ questionId: "1" });
        const option1 = question1?.options.find(
          (opt) => opt.optionId === option
        );
        q1OutcomePoints = option1 ? option1.outcomePoint : 0;
      }
    ...
    const averageOutcomePoints =
      totalUsers > 0 ? totalOutcomePoints / totalUsers : 0;
...
const getUniversityLackOfSkillsAndPreparedness = async (req, res) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const question4Response = questionResponses.find(
        (response) => response.questionId === "4"
      );
      const question5Response = questionResponses.find(
        (response) => response.questionId === "5"
      );
      const question10Response = questionResponses.find(
        (response) => response.questionId === "10"
      );
      ...
      if (question4Response) {
        ...
        const question4 = await db
          .collection("questions")
          .findOne({ questionId: "4" });
        const option4 = question4?.options.find(
          (opt) => opt.optionId === option
        );
        q4OutcomePoints = option4 ? option4.outcomePoint : 0;
      }
      ...
      if (question5Response) {
        ...
        const question5 = await db
          .collection("questions")
          .findOne({ questionId: "5" });
        const option5 = question5?.options.find(
          (opt) => opt.optionId === option
        );
        q5OutcomePoints = option5 ? option5.outcomePoint : 0;
      }
    ...
    res.status(200).json({
      message:
        "University outcome points for Lack of Skills and Preparedness calculated successfully",
      data: {
        universityId,
        totalUsers,
        totalOutcomePoints,
...
const getUniversityConfusionAboutBranchesAndAlignment = async (req, res) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const question6Response = questionResponses.find(
        (response) => response.questionId === "6"
      );
      const question13Response = questionResponses.find(
        (response) => response.questionId === "13"
      );
      ...
      if (question6Response) {
        ...
        const question6 = await db
          .collection("questions")
          .findOne({ questionId: "6" });
        const option6 = question6?.options.find(
          (opt) => opt.optionId === option
        );
        q6OutcomePoints = option6 ? option6.outcomePoint : 0;
      }
    ...
    res.status(200).json({
      message:
        "University outcome points for Confusion About Branches & Alignment calculated successfully",
      data: {
        universityId,
        totalUsers,
        totalOutcomePoints,
...
const getUniversityInternshipSelectionForJobReadiness = async (req, res) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const question9Response = questionResponses.find(
        (response) => response.questionId === "9"
      );
      const question10Response = questionResponses.find(
        (response) => response.questionId === "10"
      );
      const question12Response = questionResponses.find(
        (response) => response.questionId === "12"
      );
      ...
      if (question9Response) {
        ...
        for (let optionId of question9Response.optionSelected) {
          const option9 = question9?.options.find(
            (opt) => opt.optionId === optionId
          );
          q9OutcomePoints += option9 ? option9.outcomePoint : 0;
        }
    ...
    res.status(200).json({
      message:
        "University outcome points for Internship Selection for Job Readiness calculated successfully",
      data: {
        universityId,
        totalUsers,
        totalOutcomePoints,
...
const getAllLackOfSkillsAndPreparedness = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Outcome points for Lack of Skills and Preparedness calculated successfully for all users",
      data: {
        totalUsers,
        totalOutcomePoints,
        averageOutcomePoints,
    ...
    console.error(
      "Error fetching outcome points for Lack of Skills and Preparedness:",
      error
    );
    res.status(500).json({ message: "Server error" });
  }
...
const getAllConfusionAboutBranchesAndAlignment = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Outcome points for Confusion About Branches and Alignment calculated successfully for all users",
      data: {
        totalUsers,
        totalOutcomePoints,
        averageOutcomePoints,
    ...
    console.error(
      "Error fetching outcome points for Confusion About Branches and Alignment:",
      error
    );
    res.status(500).json({ message: "Server error" });
  }
...
const getAllInternshipSelectionForJobReadiness = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Outcome points for Internship Selection for Job Readiness calculated successfully for all users",
      data: {
        totalUsers,
        totalOutcomePoints,
        averageOutcomePoints,
    ...
    console.error(
      "Error fetching outcome points for Internship Selection for Job Readiness:",
      error
    );
    res.status(500).json({ message: "Server error" });
  }
...
const getUniversityMismatchSalaryExpectations = async (req, res) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const question7Response = questionResponses.find(
        (response) => response.questionId === "7"
      );
      const question8Response = questionResponses.find(
        (response) => response.questionId === "8"
      );
      ...
      if (question7Response) {
        ...
        const question7 = await db
          .collection("questions")
          .findOne({ questionId: "7" });
        const option7 = question7?.options.find(
          (opt) => opt.optionId === option
        );
        q7OutcomePoints = option7 ? option7.outcomePoint : 0;
      }
      ...
      if (question8Response) {
        ...
        const question8 = await db
          .collection("questions")
          .findOne({ questionId: "8" });
        const option8 = question8?.options.find(
          (opt) => opt.optionId === option
        );
        q8OutcomePoints = option8 ? option8.outcomePoint : 0;
      }
    ...
    res.status(200).json({
      message:
        "University outcome points for Mismatch Salary Expectations calculated successfully",
      data: {
        universityId,
        totalUsers,
        totalOutcomePoints,
...
const getUniversityInterestCareerSupportServices = async (req, res) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      const question11Response = questionResponses.find(
        (response) => response.questionId === "11"
      );
      const question13Response = questionResponses.find(
        (response) => response.questionId === "13"
    ...
    res.status(200).json({
      message:
        "University outcome points for Interest Career Support Services calculated successfully",
      data: {
        universityId,
        totalUsers,
        totalOutcomePoints,
...
const getAllMismatchSalaryExpectations = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Outcome points for Mismatch Salary Expectations calculated successfully for all users",
      data: {
        totalUsers,
        totalOutcomePoints,
        averageOutcomePoints,
...
const getAllInterestCareerSupportServices = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message:
        "Outcome points for Interest Career Support Services calculated successfully for all users",
      data: {
        totalUsers,
        totalOutcomePoints,
        averageOutcomePoints,
...
const getCounterByUniversityWise = async (req, res) => {
  try {
    const db = getDb();
    const { universityId } = req.params; // Retrieve ID from route parameters
    ...
    // Query the database for users with the given university ID
    const users = await db.collection("users").find({ school: new ObjectId(universityId) }).toArray();
    ...
    const totalResponse = users.length;

    // Count undergraduate and postgraduate courses
    const undergraduate = users.filter(user => user.course === "undergraduate").length;
    const postgraduate = users.filter(user => user.course === "postgraduate").length;
...
module.exports = {
  ...
  getQuestionById,
  storeAnswerById,
  getSurveyResultsByQuestionId,
  getSurveyStatistics,
  getLimitedUnderstandingJobOpportunities,
  getLackOfSkillsAndPreparedness,
  getConfusionAboutBranchesAndAlignment,
  getInternshipSelectionForJobReadiness,
  getCombinedOutcomePoints,
  getUniversityLimitedUnderstandingJobOpportunities,
  getUniversityLackOfSkillsAndPreparedness,
  getUniversityConfusionAboutBranchesAndAlignment,
  getUniversityInternshipSelectionForJobReadiness,
  getAllLimitedUnderstandingJobOpportunities,
  getAllLackOfSkillsAndPreparedness,
  getAllConfusionAboutBranchesAndAlignment,
  getAllInternshipSelectionForJobReadiness,
  getUniversityMismatchSalaryExpectations,
  getUniversityInterestCareerSupportServices,
  getAllMismatchSalaryExpectations,
  getAllInterestCareerSupportServices,
  getMismatchSalaryExpectations,
  getInterestCareerSupportServices,
  getAllSurveyResultsByQuestionId,
  getUniversitySurveyResultsByQuestionId,
  getMostPreferredCareerChoices,
  getLeastPreferredCareerChoices,
